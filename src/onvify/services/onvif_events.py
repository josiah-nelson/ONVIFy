...
def parse_subscribe_request(body: str) -> dict[str, Any]:
    ...
    callback_match = re.search(r"<\w+:?Address[^>]*>(https?://[^<]+)</\w+:?Address>", body)
    ttl_match = re.search(r"<\w+:?InitialTerminationTime[^>]*>(.*?)</\w+:?InitialTerminationTime>", body)

    result: dict[str, Any] = {}
    if callback_match:
        result["callback_url"] = callback_match.group(1).strip()
    if ttl_match:
        result["ttl_raw"] = ttl_match.group(1).strip()
    return result
